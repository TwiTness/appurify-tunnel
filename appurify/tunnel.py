...
class HttpParser(object):
    ...
    def process(self, data):
        if self.state >= HTTP_PARSER_STATE_HEADERS_COMPLETE and (self.method == "POST" or self.type == HTTP_RESPONSE_PARSER):
            if not self.body:
                self.body = ''
        ...
        if self.state == HTTP_PARSER_STATE_HEADERS_COMPLETE and self.type == HTTP_REQUEST_PARSER and not self.method == "POST" and self.raw.endswith(CRLF*2):
            self.state = HTTP_PARSER_STATE_COMPLETE
...
class Proxy(threading.Thread):
    ...
    def process(self):
        while True:
            ...
            if self.server:
                rlist.append(self.server)
            if self.server and len(self.buffer['server']) > 0:
                wlist.append(self.server)
...
class Tunnel(object):
    ...
    def cli():
        ...
        if (args.api_key is None or args.api_secret is None) and (args.username is None or args.password is None):
